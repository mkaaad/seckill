...
### 核心特性
- **Lua 原子扣库存**：`DECR` + 不足回滚在同一脚本内完成，避免竞态
- **令牌桶限流**：Redis + Lua 每用户桶（默认 capacity=5 突发，rate=1 token/s 补充），原子取令牌
- **异步订单处理**：Kafka 削峰；Create 侧 **SyncProducer 单例**，发送失败回补库存
- **消费幂等**：MySQL `(user_id, product_id)` 唯一索引；重复消息不重复插单、不重复回补库存
- **订单状态缓存**：pending → 落库成功 DEL 回源 success；写库失败 SET failed + 回补库存
- **热冷路径分离**：Create 扛热点，Store 只负责落库
...
尝试在秒杀期间购买商品。包含**令牌桶限流**（每用户 capacity=5、rate=1 token/s）和原子库存检查。
